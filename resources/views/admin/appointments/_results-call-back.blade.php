...
<div class="s-card shadow">
    ...
    <div class="s-card-header border-0">
        <div class="row ">
            <div class="col-9">
                {{ $prospects->links() }}
                <span class="float-start badge bg-brand text-white px-1 fs-90 mt-1">
        ...
        @foreach($prospects as $prospect)
            <div class="row s-card-row line-height-30">
                ...
                <div class="col-4 col-lg-2 border-right border-light text-start text-lg-start">
                    ...
                    <a href="{{route('admin.edit.appointment', [$prospect->event_id, $dealership->id, $prospect->id])}}" class="float-end btn btn-border sister text-center px-1 py-0 mt-1 js-admin-book" title="Edit {{$prospect->title}} {{$prospect->name}} {{$prospect->surname}} Appointment">
                ...
                <div class="col-4 col-lg-1 text-center border-right">
                        ...
                        @else
                            @if(is_null($prospect->appointment->call_attempts))
                                ----
                            @else
                                {{$prospect->appointment->call_attempts}}
                            @endif
                        @endif
                </div>
                {{--END call Status--}}
                ...
                <div class="col-4 col-lg-2 text-center border-right">
                    <span class="booked-circle border border-warning" title="Appointment In Progress"></span>
                </div>
                {{--END Booking Status--}}
