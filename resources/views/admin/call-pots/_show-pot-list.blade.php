            ...
            <div class="col pt-1">
                @if(isNow() > isPast($campaign->end_date))
                    <span class="fs-80 mr-3 alert-danger text-danger py-1 px-2 rounded">
                        Campaign ended at: <span class="ml-1"> {{ formatDate(isPast($campaign->end_date))}}</span>
                    </span>
                @else
                    <span class="fs-80 mr-3 bg-white text-brand px-2 py-1 rounded">
                        Campaign ends at: <span class="ml-1"> {{ formatDate(isPast($campaign->end_date))}}</span>
                    </span>
                @endif
            </div>
            <div class="col text-end pt-1">
                @admin('super-admin')
                    @if(count($pot_lists) > 0)
                        <a href="{{route('admin.pot-list.export', $campaign->id)}}" class="btn btn-sm btn-default sister px-2 mr-3"> Export League <i class="fas fa-file-download ml-2"></i></a>
                    @endif
                @endadmin
                    ...
                    <div class="row px-1 s-card-row py-1 line-height-34">
                        ...
                        @if($list->locked == 1 && $list->admin_id == auth('admin')->user()->id)
                            <div class="col text-end pe-1" style="">
                                <a href="javascript:void(0)" class="btn btn-sm btn-default success js-action px-1" title="This Pot List Belongs to you" item-id="{{$list->id}}"><i class="fa-solid fa-check-to-slot"></i></a>
                            </div>
                        @elseif($list->locked == 0)
                            <div class="col text-end pe-1" style="">
                                <a href="javascript:void(0)" class="btn btn-sm btn-default info js-action" title="Register Action" item-id="{{$list->id}}"><i class="fa-solid fa-keyboard"></i></a>
                            </div>
                        @else
                            <div class="col text-end pe-1" style="">
