    <div class="s-card-header bg-white">
        <div class="row ">
            <div class="col-9">
                {{ $prospects->links() }}
                <span class="float-start badge bg-brand text-white px-1 fs-90">
                    Results: {{number_format($prospects->count())}}
                </span>
            </div>

            <div class="col-3 pr-3 text-end">
                ...
                <div class="row s-card-row line-height-30">
                    ...
                    <div class="col-4 col-lg-2 border-right border-light text-start text-lg-start">
                        ...
                        @if(prospectIsBooked($prospect->id))
                            <a href="{{route('exec.edit.appointment', [$prospect->event_id, $dealership->id, $prospect->id])}}" class="mt-1 float-end btn btn-border brother text-center px-1 py-0 js-admin-book" title="Edit {{$prospect->title}} {{$prospect->name}} {{$prospect->surname}} Appointment">
                                <i class="far fa-edit fs-110"></i>
                            </a>
                        @else
                            <a href="{{route('exec.create.appointment', [$prospect->event_id, $dealership->id, $prospect->id])}}" class="mt-1 float-end btn btn-border sister text-center px-1 py-0 js-admin-book" title="Book {{$prospect->title}} {{$prospect->name}} {{$prospect->surname}}">
                    ...
                    {{-- Booking Status--}}
                        <div class="col-4 col-lg-2 text-center border-right">
                            @if(isset($prospect->appointment->id))
                                ...
                                @if($prospect->appointment->confirm == 0 &&  $prospect->appointment->cancelled == 0  &&  $prospect->appointment->not_interested == 0)
                                    @if($prospect->appointment->booked_by == NULL && $prospect->appointment->edited_by == NULL)
                                        <span class="icon booked-circle alert-danger" title="Hot Prospect"></span>
                                    @else
