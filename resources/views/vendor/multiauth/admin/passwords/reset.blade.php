@extends('_layouts._login')


@section('content')
<div class="container">
    <div class="row justify-content-center">
        <div class="col-lg-10">
            <div class="s-card shadow text-dark">
                <h1 class="s-card-header mb-0 alert-brother border-0 px-4 fs-150">{{ ucfirst(config('multiauth.prefix')) }} Reset Password</h1>

                <div class="s-card-body px-4 fs-110">
                    <div class="p-3 alert-warning lighter rounded bold mb-3">
                        Your password  needs to have a minimum of: <br>
                        10 characters, have a number, special character, uppercase and lowercase.
                    </div>

                    <form method="POST" action="{{ route('admin.password.request') }}" aria-label="{{ __('Admin Reset Password') }}">
                        @csrf

                        <input type="hidden" name="token" value="{{ $token }}">

                        <div class="form-group ">
                            <label for="email" class="bold">{{ __('E-Mail Address') }}</label>
                            <input id="email" type="email" class="form-control form-control-lg {{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ $email ?? old('email') }}"
                                required autofocus> @if ($errors->has('email'))
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $errors->first('email') }}</strong>
                                </span> @endif
                        </div>

                        <div class="form-group relative">
                            <label for="password" class="bold">{{ __('Password') }}</label>
                            <input id="password" type="password" class="form-control form-control-lg {{ $errors->has('password') ? ' is-invalid' : '' }}" name="password"
                                required>
                            <i class="fas fa-eye js-toggle-password fs-150"></i>
                            @if ($errors->has('password'))
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $errors->first('password') }}</strong>
                                </span>
                            @endif
                        </div>

                        <div class="form-group relative">
                            <label for="password-confirm" class="bold">{{ __('Confirm Password') }}</label>
                            <input id="password-confirm" type="password" class="form-control form-control-lg" name="password_confirmation" required>
                            <i class="fas fa-eye js-toggle-password-confirm fs-150"></i>
                        </div>

                        <div class="form-group mt-3 mb-0">
                            <button type="submit" class="btn btn-default sister">
                                {{ __('Reset Password') }} <i class="fa fa-angle-double-right"></i>
                            </button>
                        </div>

                    </form>
                </div>
            </div>
        </div>
    </div>
</div>
@endsection
